...
상품 사진과 배너는 이미지 모델로 만든 것입니다(실제 상품이 아닙니다). 포장의 우유랩스 로고와 제품명은 로고 파일을 참조로 주고 사진 자체를 생성한 것입니다. **상품은 모두 정면에서 찍습니다** — 비스듬히 놓으면 포장의 인쇄만 정면으로 그려져 스티커처럼 떠 보입니다. 새 상품을 더할 때도 같은 각도·같은 배경으로 만듭니다.
