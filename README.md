...
상품 사진과 배너는 이미지 모델로 만든 것입니다(실제 상품이 아닙니다). 포장의 그림과 글자도 사진과 함께 생성한 것이라 **표식은 우유랩스 로고를 닮게 그린 것이지 로고 파일 그대로가 아닙니다** — 로고 파일을 사진 위에 합성해 보았더니 조명과 종이 결이 맞지 않아 스티커처럼 떠 보였습니다. 새 상품을 더할 때 지킬 것은 넷입니다: 같은 배경과 조명, **우유갑은 비스듬히·통과 병은 정면으로**(원통은 돌리면 라벨의 글자가 깨집니다), 인쇄가 포장 면의 각도를 따라갈 것, **표식은 작게**(포장 너비의 5분의 1 안쪽) 두고 제품명을 가장 크게 둘 것.
